...
export default {
  ...
  render(h) {
    ...
    this.$nextTick().then(() => {
      this.handlerPrefix(
        slots,
        this.showSpin ? this.addSkeletPrefix : this.removeSkeletPrefix,
        this.init,
      );
    });
  ...
  methods: {
    ...
    handlerPrefix(slots, handler, init = true) {
      slots.forEach((slot) => {
        const children = slot.children
          ...
          || ((slot.componentInstance || {})._vnode || {}).children;
        if (!slot.componentOptions) {
          ...
          (function (slot) {
            ...
            const { insert } = slot.data.hook || {};
            (slot.data.hook || {}).insert = () => {
              // 函数重构, 修改原有的组件hook, 并且保证insert只执行一次
              insert(slot);
              handlerComponent();
            };
    ...
    handlerComponent(slot, handler, init) {
      const originchildren = (((slot.componentInstance || {})._vnode || {}).componentOptions || {})
        .children;
    ...
    removeSkeletPrefix(slot) {
      ...
      if (rootVnode.elm && rootVnode.elm.nodeType === 1) {
        if (rootVnode.elm) {
          ...
          rootVnode.data.staticClass = rootVnode.data.staticClass.replace(
            ` ${this.skeletPrefix}`,
            '',
          );
        }
